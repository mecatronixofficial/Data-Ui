          ...
          >
            <div className="py-1 flex flex-row items-center justify-between">
              <div className="mb-1 flex items-center gap-2">
                <span className="flex h-6 w-6 items-center justify-center rounded-lg bg-blue-600 font-display text-xs text-white shadow-md shadow-blue-900/15">
                  {field.icon ? <FieldIcon icon={field.icon} size={11} /> : String(fieldIndex + 1).padStart(2, '0')}
                </span>
                <div>
                  <h2 className="font-display text-base text-blue-950">{field.name}</h2>
                </div>
              </div>
              <div>
  ...
  return (
    <section className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-950 via-blue-800 to-blue-600 p-4 text-white shadow-[0_20px_50px_rgba(0,107,196,0.25)]">
      ...
      <div className="relative mb-3 flex items-center gap-2">
        <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-white/15 font-display text-sm text-white">
          {String(fieldTotals.length + 1).padStart(2, '0')}
        </span>
        <div>
          <h2 className="font-display text-base">Final Total</h2>
        </div>
      </div>

      {!single && (
        <div className="relative flex flex-wrap items-end justify-center gap-2 py-1">
          {fieldTotals.map((total, index) => (
            <span key={index} className="flex items-end gap-2">
              <TotalPill label={`${fieldNames[index]} Total`} value={total} dark colorBySign />
              {index < fieldTotals.length - 1 && (
                <span className="pb-1 font-display text-sm text-blue-50/70">+</span>
              )}
            </span>
          ))}
        </div>
      ...
      <div className="relative mt-3 flex items-center justify-between border-t border-dashed border-white/20 pt-3">
        <span className="text-xs uppercase tracking-widest text-blue-50/60">Final Total</span>
        <span
          className={`font-mono text-2xl font-semibold tabular ${finalTotal < 0 ? 'text-red-300' : finalTotal > 0 ? 'text-blue-200' : 'text-white'
            }`}
        >
          {finalTotal}
        </span>
