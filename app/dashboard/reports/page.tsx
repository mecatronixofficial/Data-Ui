...
type EntryField = {
  name: string; boxNames: string[]; boxes: number[]; details: BoxDetail[][];
...
export default function ReportsPage() {
  ...
  function startEdit(entry: Entry) {
    setEditing({
      ...
      fields: entry.fields.map((f) => ({
        name: f.name, boxNames: [...f.boxNames], boxes: [...f.boxes],
  ...
  function updateEditingFieldOperator(fieldIndex: number, op: Operator) {
    setEditing((cur) => { if (!cur) return cur; const fs = [...cur.fields]; fs[fieldIndex] = { ...fs[fieldIndex], operator: op }; return { ...cur, fields: fs }; });
  }
    ...
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-950 via-blue-800 to-blue-600 px-8 py-7 text-white shadow-[0_20px_50px_rgba(0,107,196,0.30)]">
        ...
        <div className="relative flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="mb-1 text-[10px] font-semibold uppercase tracking-[0.22em] text-blue-200/60">Overview</p>
            <h1 className="font-display text-3xl drop-shadow-[0_2px_8px_rgba(0,0,0,0.25)] sm:text-4xl">Reports</h1>
          </div>
