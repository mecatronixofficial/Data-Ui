    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex items-center gap-3 rounded-2xl border border-blue-100 bg-white px-4 py-3 shadow-card">
          ...
          <div>
            <p className="font-display text-xl leading-none text-blue-950">{users.length}</p>
            <p className="mt-1 text-[10px] font-semibold uppercase tracking-wider text-blue-900/45">Total users</p>
          </div>
        </div>
      </div>
      ...
      <form
        ...
        className="grid grid-cols-1 items-end gap-5 rounded-2xl border border-blue-100 bg-white p-6 shadow-[0_14px_40px_rgba(0,107,196,0.08)] sm:grid-cols-2 lg:grid-cols-4"
      >
        <div className="sm:col-span-2 lg:col-span-4">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-blue-600 text-white shadow-lg shadow-blue-900/15">
        ...
        <div className="sm:col-span-2 lg:col-span-4">
          <button
            type="submit"
            className="flex items-center gap-2 rounded-xl bg-blue-600 px-5 py-2.5 font-medium text-white shadow-lg shadow-blue-900/15 transition hover:-translate-y-0.5 hover:bg-blue-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2"
          >
      ...
      {success && <p className="flex items-center gap-2 rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-700"><FiCheckCircle />{success}</p>}
      ...
      <div className="overflow-hidden rounded-2xl border border-blue-100 bg-white shadow-[0_14px_40px_rgba(0,107,196,0.08)]">
        ...
        <table className="w-full min-w-[620px] text-sm">
          ...
          <tbody>
            {loading && (
              <tr>
                <td colSpan={4} className="px-5 py-8 text-center text-blue-900/40 font-mono text-xs">
                  loading…
                </td>
              </tr>
            )}
            {!loading &&
              users.map((u) => (
                <tr key={u._id} className="border-b border-blue-50 transition last:border-0 hover:bg-blue-50/45">
