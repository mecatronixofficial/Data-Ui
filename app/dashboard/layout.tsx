...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  if (loading) {
    return (
      <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-gradient-to-br from-blue-950 via-blue-800 to-blue-600">
        <div className="absolute -left-24 -top-24 h-72 w-72 rounded-full bg-blue-300/15 blur-3xl" />
        ...
        <div className="relative flex flex-col items-center rounded-2xl border border-white/15 bg-white/10 px-10 py-8 shadow-2xl backdrop-blur-md">
          ...
          <p className="font-mono text-xs uppercase tracking-[0.25em] text-blue-50/80">Loading workspace</p>
        </div>
      </div>
    );
  }
